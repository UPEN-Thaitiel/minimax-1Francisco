BOARD_SIZE = 3
REWARD = 10

class TicTacToe:
    def __init__(self, board):
        self.board = board
        self.player = 'O'
        self.computer = 'X'

    def run(self):
        print("¡Bienvenido a Tic-Tac-Toe! Tú eres 'O'. La computadora es 'X'.")
        self.print_board()
        self.move_computer()
        while True:
            self.move_player()
            self.move_computer()

    def print_board(self):
        for i in range(1, 10, 3):
            print(f" {self.board[i]} | {self.board[i+1]} | {self.board[i+2]} ")
            if i < 7:
                print("---+---+---")

    def is_cell_free(self, position):
        return self.board[position] == ' '

    def update_player_position(self, player, position):
        if self.is_cell_free(position):
            self.board[position] = player
            self.check_game_state()
        else:
            if player == self.player:
                print("Esa celda ya está ocupada. Intenta con otra.")
                self.move_player()

    def is_winning(self, player):
        win_combinations = [
            [1, 2, 3], [4, 5, 6], [7, 8, 9],  # filas
            [1, 4, 7], [2, 5, 8], [3, 6, 9],  # columnas
            [1, 5, 9], [3, 5, 7]              # diagonales
        ]
        return any(all(self.board[pos] == player for pos in combo) for combo in win_combinations)

    def is_draw(self):
        return all(self.board[pos] != ' ' for pos in self.board) and not self.is_winning(self.player) and not self.is_winning(self.computer)

    def check_game_state(self):
        self.print_board()
        if self.is_winning(self.player):
            print("¡Ganaste!")
            exit()
        elif self.is_winning(self.computer):
            print("La computadora gana.")
            exit()
        elif self.is_draw():
            print("¡Empate!")
            exit()

    def move_player(self):
        while True:
            try:
                position = int(input("Tu turno. Elige una posición (1-9): "))
                if 1 <= position <= 9:
                    if self.is_cell_free(position):
                        self.update_player_position(self.player, position)
                        break
                    else:
                        print("Esa celda está ocupada.")
                else:
                    print("Por favor elige un número entre 1 y 9.")
            except ValueError:
                print("Entrada inválida. Introduce un número.")

    def move_computer(self):
        best_score = float('-inf')
        best_move = None
        for pos in range(1, 10):
            if self.is_cell_free(pos):
                self.board[pos] = self.computer
                score = self.minimax(0, False)
                self.board[pos] = ' '
                if score > best_score:
                    best_score = score
                    best_move = pos
        self.update_player_position(self.computer, best_move)

    def minimax(self, depth, is_maximizer):
        if self.is_winning(self.computer):
            return REWARD - depth
        if self.is_winning(self.player):
            return -REWARD + depth
        if self.is_draw():
            return 0

        if is_maximizer:
            best_score = float('-inf')
            for pos in range(1, 10):
                if self.is_cell_free(pos):
                    self.board[pos] = self.computer
                    score = self.minimax(depth + 1, False)
                    self.board[pos] = ' '
                    best_score = max(score, best_score)
            return best_score
        else:
            best_score = float('inf')
            for pos in range(1, 10):
                if self.is_cell_free(pos):
                    self.board[pos] = self.player
                    score = self.minimax(depth + 1, True)
                    self.board[pos] = ' '
                    best_score = min(score, best_score)
            return best_score

if __name__ == '__main__':
    board = {pos: ' ' for pos in range(1, 10)}
    game = TicTacToe(board)
    game.run()
