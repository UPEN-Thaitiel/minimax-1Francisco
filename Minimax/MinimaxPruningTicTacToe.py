...
class TicTacToe:
    ...
    def run(self):
        print("¡Bienvenido al juego Tic-Tac-Toe! Tú eres 'O', la computadora es 'X'.")
        self.move_computer()
        while True:
            self.move_player()
            self.move_computer()
    ...
    def update_player_position(self, player, position):
        if self.is_cell_free(position):
            ...
            if player == self.player:
                print("Esa posición ya está ocupada. Intenta con otra.")
                self.move_player()
    ...
    def move_player(self):
        while True:
            try:
                position = int(input("Elige tu posición (1-9): "))
                if 1 <= position <= 9:
